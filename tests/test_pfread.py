...
def test0_plantrial0_read():
    trial0 = PlanTrial0['Trial'][0]
    assert (trial0['Name'],
            trial0['DoseGridVoxelSizeX'],
            trial0['DoseGridOriginY'],
            trial0['PrescriptionList']['Prescription'][0]['PrescriptionDose'],
            trial0['BeamList']['Beam'][0]['IsocenterName'],
            trial0['BeamList']['Beam'][0]['DosePerMuAtPrescriptionPoint'],
            trial0['BeamList']['Beam'][0]['MachineNameAndVersion'],
            trial0['BeamList']['Beam'][0]['Modality'],
            trial0['BeamList']['Beam'][0]['MachineEnergyName'],
            trial0['BeamList']['Beam'][0]['SetBeamType']
        ) == (
            'sMLC',
            '0.4',
            '-83.7535',
...
def test1_plantrial0_controlpoint_read():
    cp0 = PlanTrial0['Trial'][0]['BeamList']['Beam'][0]['CPManager']['CPManagerObject'][0]['ControlPointList']['ControlPoint'][1]
    assert (cp0['Gantry'],
            cp0['LeftJawPosition'],
            ...
            cp0['ModifierList']['BeamModifier'][0]['ContourList']['CurvePainter'][1]['Curve']['RawData']['Points'][1],
            cp0['MLCLeafPositions']['RawData']['NumberOfPoints'],
            cp0['MLCLeafPositions']['RawData']['Points'][30],
            cp0['MLCLeafPositions']['RawData']['Points'][95],
...
def test1_plantrial1_controlpoint_read():
    cp0 = PlanTrial1['Trial'][0]['BeamList']['Beam'][0]['CPManager']['CPManagerObject'][0]['ControlPointList']['ControlPoint'][0]
    assert (cp0['Gantry'],
            cp0['LeftJawPosition'],
            cp0['WedgeContext']['WedgeName'],
            cp0['WedgeContext']['Orientation'],
...
def test2_plantrial0_controlpoint_read():
    beam1 = PlanTrial0['Trial'][0]['BeamList']['Beam'][1]
    assert (beam1['UseMLC'],
...
def test2_plantrial1_controlpoint_read():
    beam0 = PlanTrial1['Trial'][0]['BeamList']['Beam'][0]
    assert (beam0['UseMLC'],
